...
Thanks to Aashita Kesarwani for providing this data source to NYT articles and comments for March 2018.

The data analysed below comes from NYT bylines and comments for March 2018. I will carry out some high level Exploratory Data Analysis (EDA), and then finish off by trying to understand if keywords in a byline could influence how many comments an article can get. I have recently completed a DataCamp course on the `tidytext` package and wanted to put my practice into a kernal - just a newbie trying to get better!
...
# load in data
articles_march18 <- read.csv("/Users/jasonzivkovic/Documents/NewYorkTimesTextAnalysis/NYTArticlesMarch2018.csv", stringsAsFactors = F)
comments_march18 <- read.csv("/Users/jasonzivkovic/Documents/NewYorkTimesTextAnalysis/NYTCommentsMarch2018.csv", stringsAsFactors = F)
...
ggplot(data = articles_march18, aes(x= articleWordCount)) +
  geom_histogram(col = "darkblue", fill = "white") +
  labs(x="Word Count", y="")+
  theme_minimal()
...
Of the top two types of material (News and Op-Ed), Op-Eds receive more comments on avearge (median *Op-Ed* = `r median(articles_march18$num_comments[articles_march18$typeOfMaterial == "Op-Ed"])` to median *News* = `r median(articles_march18$num_comments[articles_march18$typeOfMaterial == "Op-Ed"])`
...
```{r, message=FALSE, warning=FALSE}
# create a data frame to use in the Trump lm
...
That's all for this kernal, Thank you for reading (if you have gotten this far)! 

I will turn my attention in my next kernal to the body of the articles and what the readers are saying in their comments. 